...
const btnRemovarTodas = document.querySelector("#btn-remover-todas");
...
function criarElementoTarefa(tarefa) {
  ...
  botao.onclick = () => {
    ...
    if (novaDescrição) {
      ...
      tarefa.descrição = novaDescrição;
      atualizarTarefas();
    }
  };
...
formAdicionarTarefa.addEventListener("submit", (evento) => {
  ...
  // criação do objeto que sera armazenado no arrey
  const tarefa = {
    descrição: textarea.value,
  };
  tarefas.push(tarefa); // adiciona o objeto no arrey
...
btnRemovarTodas.onclick = () => removerTarefas(false);
